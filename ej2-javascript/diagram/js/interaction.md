...
| Tab | Tab to Focus | Focus on the diagram element based on the rendering order when using the "Tab" key.|
...
| Control + R | Rotate anti-clockwise | Rotate the selected nodes in clockwise.|
| Control + L | Rotate clockwise | Rotate the selected nodes in counterclockwise.|
...
| Control + 6 | Line tool | Activate the line tool.|
